...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_sq = (x_train_flat ** 2).sum(dim=1)[:, None]   # shape (num_train, 1)
    test_sq = (x_test_flat ** 2).sum(dim=1)[None, :]     # shape (1, num_test)

    cross = x_train_flat @ x_test_flat.T                  # shape (num_train, num_test)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    # Replace "pass" statement with your code
    for j in range(num_test):
        _, idx = torch.topk(dists[:, j], k=k, largest=False)
...
def knn_cross_validate(
    ...
    for fold in range(num_folds):
        ...
        x_train_cv = torch.cat([x_train_folds[i] for i in range(num_folds) if i != fold])
        y_train_cv = torch.cat([y_train_folds[i] for i in range(num_folds) if i != fold])

...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_pre = X @ W1 + b1  # (N, H)

    # ReLU 激活
    hidden = torch.maximum(hidden_pre, torch.zeros_like(hidden_pre))  # (N, H)
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # 使用 loss_func 计算 scores
    scores = loss_func(params, X)  # 如果 y=None，loss_func 返回 scores
...
def find_best_net(
    ...
    N, D = X_train.shape
    C = y_train.max().item() + 1  # 类别数

    learning_rates, hidden_sizes, regs, lr_decays = get_param_set_fn()
        ...
        for hs in hidden_sizes:
            for reg in regs:
                for decay in lr_decays:
                    # 初始化 TwoLayerNet
                    net = TwoLayerNet(input_size=D, hidden_size=hs, output_size=C)

                    # 训练网络
                    stats = net.train(
                        X_train, y_train,
                        X_val, y_val,
                        num_iters=1000,       # 可以根据数据集大小调整
                        batch_size=200,
                        learning_rate=lr,
